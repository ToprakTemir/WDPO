...
"""Defines the available WPO configuration options."""
...
class WPOConfig:
  ...
  experience_type: wpo_types.ExperienceType = dataclasses.field(
      default_factory=lambda: wpo_types.FromSequences(n_step=5)
  )
  num_stacked_observations: int = 1
  # Optional data-augmentation transformation for observations.
  observation_transform: Union[Callable[
      [types.NestedTensor], types.NestedTensor], None] = None
  ...
  samples_per_insert: Union[float, None] = 32.0
  ...
  num_samples: int = 20  # Number of WPO action samples.
  policy_loss_config: Union[wpo_types.PolicyLossConfig, None] = None
  ...
  target_update_period: Union[int, None] = 100
  target_update_rate: Union[float, None] = None
  ...
  def __post_init__(self):
    ...
    if (isinstance(self.experience_type, wpo_types.FromTransitions) and
